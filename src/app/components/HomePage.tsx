    ...
    <div style={{ background: T.bg, fontFamily: T.sans }}>
      ...
      >
        ...
        <svg className="absolute inset-0 w-full h-full pointer-events-none" viewBox="0 0 1160 620" preserveAspectRatio="xMidYMid slice" aria-hidden="true">
          ...
          <polygon points={hexPoints(1120, -40, 400)} fill={T.amber} opacity="0.14" clipPath="url(#hero-clip)" />
        </svg>

        {/* Portrait — desktop: flush to the right page edge, vertically cropped */}
        <img
          src={heroPortrait}
          alt="Carla Chahwan — Strategic UX Architect"
          className="hidden lg:block absolute right-0 top-1/2 -translate-y-1/2 w-auto pointer-events-none select-none"
          style={{ maxHeight: 640, objectFit: 'contain', filter: 'drop-shadow(0 24px 60px rgba(0,0,0,0.55))' }}
        />

        <div className="relative max-w-7xl mx-auto px-6 lg:px-10 py-24 w-full">
          <div className="lg:max-w-[52%]">
            {/* Mobile portrait — sits on top, in flow */}
            <img
              src={heroPortrait}
              alt="Carla Chahwan — Strategic UX Architect"
              className="lg:hidden block w-auto mx-auto mb-10 max-h-[360px] sm:max-h-[440px] object-contain"
              style={{ objectPosition: 'bottom center', filter: 'drop-shadow(0 20px 50px rgba(0,0,0,0.5))' }}
            />
            <Eyebrow style={{ fontSize: 17, letterSpacing: '0.12em' }}>Strategic UX Architect · Beirut, Lebanon</Eyebrow>
            ...
            >
              ...
              <span style={{color: T.amber }}>That Think</span>
              <span> &amp; Scale.</span>
            </h1>
            ...
            {/* Stats */}
            <div className="flex flex-nowrap justify-between sm:justify-start gap-x-4 sm:gap-x-28 gap-y-8 mt-16 pt-12 border-t" style={{ borderColor: T.line }}>
              {[['9', 'Years Experience'], ['15+', 'Products Shipped'], ['5', 'Countries Served']].map(([num, label]) => (
                <div key={label} className="min-w-0">
                  <div style={{ fontFamily: T.serif, fontSize: 'clamp(30px, 8vw, 42px)', fontWeight: 300, color: T.text, letterSpacing: '-0.03em', lineHeight: 1 }}>{num}</div>
                  <div style={{ fontSize: 'clamp(11px, 3vw, 14px)', color: T.dim, marginTop: 8, letterSpacing: '0.08em', textTransform: 'uppercase' }}>{label}</div>
                </div>
              ))}
            </div>
          </div>
